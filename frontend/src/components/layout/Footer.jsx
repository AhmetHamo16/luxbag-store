...
const Footer = () => {
  ...
  const socialLinks = {
    whatsapp: toWhatsAppUrl(content?.contactInfo?.whatsapp),
    instagram: content?.contactInfo?.instagram || fallbackSocialLinks.instagram,
    tiktok: content?.contactInfo?.tiktok || fallbackSocialLinks.tiktok
  };

  return (
    <footer className="melora-footer bg-[var(--footer-bg)] pt-20 pb-10 text-[#fff5e7]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          ...
          <div className="col-span-1 md:col-span-1">
            ...
            <p className="mb-6 text-sm leading-relaxed text-[#d7c4aa]">
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm text-[#d7c4aa]">
              <li><Link to="/shop" className="hover:text-[#d8ad63] transition-colors duration-300">{copy.shopAll}</Link></li>
              <li><Link to="/categories" className="hover:text-[#d8ad63] transition-colors duration-300">{copy.classicCollection}</Link></li>
              <li><Link to="/about" className="hover:text-[#d8ad63] transition-colors duration-300">{copy.ourStory}</Link></li>
              <li><Link to="/contact" className="hover:text-[#d8ad63] transition-colors duration-300">{t.contact || 'Contact'}</Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm text-[#d7c4aa]">
              <li><Link to="/faq" className="hover:text-[#d8ad63] transition-colors duration-300">{copy.faq}</Link></li>
              <li><Link to="/shipping" className="hover:text-[#d8ad63] transition-colors duration-300">{copy.shippingReturns}</Link></li>
              <li><Link to="/privacy" className="hover:text-[#d8ad63] transition-colors duration-300">{copy.privacy}</Link></li>
              <li><Link to="/terms" className="hover:text-[#d8ad63] transition-colors duration-300">{copy.terms}</Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <p className="mb-4 text-sm leading-relaxed text-[#d7c4aa]">{copy.newsletterText}</p>
            <form className="flex flex-col space-y-4 relative">
              <input 
                type="email" 
                placeholder={copy.emailPlaceholder}
                className="w-full border-b border-[#6b523d] bg-transparent px-0 py-2 text-sm text-white transition-colors placeholder:text-[#9f876b] focus:border-[#d8ad63] focus:outline-none"
              />
              <button 
                type="submit" 
                className="absolute right-0 top-1 text-xs font-bold uppercase tracking-widest text-[#d8ad63] transition-colors hover:text-white"
              >
                {copy.join}
              </button>
            </form>
        ...
        {/* Footer Bottom Line */}
        <div className="flex flex-col items-center justify-between border-t border-[#6b523d]/50 pt-8 text-center text-xs text-[#b9a58d] md:flex-row md:text-left">
          <p className="mb-4 md:mb-0">
            {content?.footerText?.[language] || `© ${new Date().getFullYear()} Melora. All rights reserved.`}
          </p>
          <div className="flex space-x-6">
            <a href={socialLinks.whatsapp} target="_blank" rel="noopener noreferrer" className="text-[#d7c4aa] transition-colors duration-300 hover:text-[#25D366]" aria-label="WhatsApp">
            ...
            <a href={socialLinks.instagram} target="_blank" rel="noopener noreferrer" className="text-[#d7c4aa] transition-colors duration-300 hover:text-[#E1306C]" aria-label="Instagram">
            ...
            <a href={socialLinks.tiktok} target="_blank" rel="noopener noreferrer" className="text-[#d7c4aa] transition-colors duration-300 hover:text-white" aria-label="TikTok">
